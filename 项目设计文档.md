...
*   **通知**: 支持 Webhook 回调通知发送结果。（未完成）

### 3.5 系统管理 (System Management)
*   **账户管理**: 账户用来登陆后台，不需要分权限组。系统管理员账户的增删改查。
...
## 8. 安全考虑
*   **用户认证**: 采用 JWT 或 Session 认证。
*   **数据加密**: 数据库连接字符串、SMTP 密码等敏感信息加密存储。
*   **输入验证**: 对所有用户输入进行验证，防止 SQL 注入、XSS 等攻击。

## 9. 系统设置
*   **默认值**: 提供默认配置，用户可根据需求修改，包括发信最小间隔，最大间隔，并发数量。
*   **用户配置**: 修改账户名和密码。
