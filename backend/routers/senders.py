...
from models import Sender, SenderCreate, SenderRead, SenderUpdate
...
@router.post("/verify")
async def verify_sender_connection(sender: SenderCreate):
...
async def create_sender(*, session: Session = Depends(get_session), sender: SenderCreate):
...
def read_senders(*, session: Session = Depends(get_session), offset: int = 0, limit: int = Query(default=100, le=100)):
    senders = session.exec(select(Sender).offset(offset).limit(limit)).all()
    return senders

@router.get("/{sender_id}", response_model=SenderRead)
def read_sender(*, session: Session = Depends(get_session), sender_id: int):
...
async def update_sender(*, session: Session = Depends(get_session), sender_id: int, sender: SenderUpdate):
...
def delete_sender(*, session: Session = Depends(get_session), sender_id: int):
    sender = session.get(Sender, sender_id)
    if not sender:
        raise HTTPException(status_code=404, detail="Sender not found")
